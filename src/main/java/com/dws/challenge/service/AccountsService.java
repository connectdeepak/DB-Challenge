...
public class AccountsService {
	...
	public void transfer(String fromAccountId, String toAccountId, BigDecimal amount) {
		...
		System.out.println(" Start transfer reqeust from " + fromAccountId + " to account " + toAccountId);
		...
		/*
		 * we need to take locks in a order .Else it will go in deadlock situation. for
		 * example if 2 transfer request is coming in parallel - Request#1: transfer x
		 * dollar from A/C1 to A/C2 Request#2 - transfer x dollar from A/C2 to A/C1
		 */
		...
		try {
			notificationService.notifyAboutTransfer(fromAccount,
					String.format("Debited %s from account %s", amount, toAccountId));
		} catch (Exception e) {
		...
		try {
			notificationService.notifyAboutTransfer(toAccount,
					String.format("Credited %s to account %s", amount, fromAccountId));
		} catch (Exception e) {
			log.error("Failed to notify Credit transaction", e);
		}
		
		System.out.println(" End transfer reqeust from " + fromAccountId + " to account " + toAccountId);
